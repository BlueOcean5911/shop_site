...
function Shirt(props) {
  ...
  const textTexture = generateTextTexture("Hello world");
  ...
  return (
    <mesh castShadow geometry={nodes.T_Shirt_male.geometry} material={materials.lambert1} material-roughness={1} {...props} dispose={null}>
      <Decal position={snap.text.position} rotation={[0, 0, 0]} scale={snap.text.scale} map={textTexture} map-anisotropy={15} />
      <Decal position={snap.logo.position} rotation={[0, 0, 0]} scale={snap.logo.scale} map={texture} map-anisotropy={16} />
    </mesh>
  )
}
